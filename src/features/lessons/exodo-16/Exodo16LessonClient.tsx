...
import { exodo16Lesson } from "./config";
...
function lessonKey(studentId: string, suffix: string) {
  return `seminario:${studentId}:${exodo16Lesson.slug}:${suffix}`;
}
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/lesson-responses?lessonSlug=${encodeURIComponent(exodo16Lesson.slug)}&studentId=${encodeURIComponent(studentIdForSync ?? "")}`,
      ...
      <main>
        <section className="relative">
          <div className="relative h-[64vh] min-h-[420px] w-full">
            <Image
              src={exodo16Lesson.hero.image.src}
              alt={exodo16Lesson.hero.image.alt}
              fill
              priority
              unoptimized
            ...
            <div className="mx-auto flex h-full max-w-6xl items-end px-4 pb-10 sm:px-6 sm:pb-14">
              <div className="w-full max-w-3xl">
                ...
                <p className="mt-3 text-base leading-relaxed text-white/90 sm:text-lg">
                  {exodo16Lesson.hero.subtext}
                </p>

                <div className="mt-6 rounded-2xl border border-white/15 bg-white/10 p-5 backdrop-blur">
                  <div className="text-xl font-bold text-white sm:text-2xl">
                    “{exodo16Lesson.hero.quote}”
                  </div>
                  <div className="mt-1 text-sm text-white/80">João 6:35</div>
                </div>

                {exodo16Lesson.referenceMaterial ? (
        ...
        <div className="py-10 sm:py-14">
          <Section
            ...
            subtitle="Assim como o corpo precisa de alimento sempre, o espírito também precisa de nutrição diária."
          >
            <div className="grid gap-4 rounded-3xl border border-slate-200 bg-sud-gray p-5 sm:grid-cols-[1.2fr_auto] sm:items-end sm:gap-6 sm:p-7">
              <InputArea
                label={exodo16Lesson.icebreaker.question}
                value={icebreaker.state}
                onChange={icebreaker.setState}
                placeholder={exodo16Lesson.icebreaker.placeholder}
                rows={4}
              />
          ...
          <div className="mt-10 sm:mt-14">
            <Section
              id="mana"
              title={exodo16Lesson.passover.title}
              subtitle={exodo16Lesson.passover.subtitle}
            >
              <div className="grid gap-6 lg:grid-cols-[1.15fr_1fr] lg:items-stretch">
                <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                  <div className="relative h-56 w-full sm:h-72">
                    <Image
                      src={exodo16Lesson.passover.image.src}
                      alt={exodo16Lesson.passover.image.alt}
                      fill
                      unoptimized
                      className="object-cover"
                  ...
                  <div className="p-5 sm:p-6">
                    ...
                    <div className="mt-1 text-base font-bold text-slate-900">
                      Confiar no Senhor diariamente
                    </div>
                    <p className="mt-2 text-sm leading-relaxed text-slate-600">
                      Leia Êxodo 16 e marque o que o Senhor promete, o que Ele
                      provê e o que pede em troca.
                    </p>
                  </div>
                </div>

                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
                  {exodo16Lesson.passover.cards.map((card) => (
                    <Card
                      key={card.key}
                      title={card.title}
          ...
          <div className="mt-10 sm:mt-14">
            <Section
              id="pao-da-vida"
              title={exodo16Lesson.sacrament.title}
              subtitle={exodo16Lesson.sacrament.subtitle}
              accent="green"
            >
              <div className="grid gap-6 lg:grid-cols-[1fr_1.15fr] lg:items-stretch">
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
                  {exodo16Lesson.sacrament.cards.map((card) => (
                    <Card
                      key={card.key}
                      title={card.title}
                      description={card.description}
                ...
                <div className="relative overflow-hidden rounded-3xl border border-sud-green/20 bg-white shadow-sm ring-1 ring-sud-green/10">
                  <div className="relative h-56 w-full sm:h-72">
                    <Image
                      src={exodo16Lesson.sacrament.image.src}
                      alt={exodo16Lesson.sacrament.image.alt}
                      fill
                      unoptimized
                      className="object-cover"
                      sizes="(max-width: 1024px) 100vw, 50vw"
                  ...
                  <div className="p-5 sm:p-6">
                    ...
                    <div className="mt-1 text-base font-bold text-slate-900">
                      Qual é seu “maná” de hoje?
                    </div>
                    <p className="mt-2 text-sm leading-relaxed text-slate-600">
                      Escolha uma ação pequena e constante para se aproximar de
                      Cristo hoje.
                    </p>
                  </div>
                </div>
          ...
          <div className="mt-10 sm:mt-14">
            <Section
              id="discussao"
              title={exodo16Lesson.discussion.title}
              subtitle={exodo16Lesson.discussion.subtitle}
            >
              <div className="grid gap-5 lg:grid-cols-[1fr_1fr] lg:items-start">
                <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
                  ...
                  <div className="mt-3 space-y-3">
                    {exodo16Lesson.discussion.questions.map((q) => (
                      <div
                        key={q}
          ...
          <div className="mt-10 sm:mt-14">
            <Section
              id="plano"
              title={exodo16Lesson.actionPlan.title}
              subtitle={exodo16Lesson.actionPlan.subtitle}
              accent="green"
            >
              <div className="rounded-3xl border border-sud-green/20 bg-white p-5 shadow-sm ring-1 ring-sud-green/10 sm:p-7">
                <div className="grid gap-4 lg:grid-cols-3">
                  <InputArea
                    label={exodo16Lesson.actionPlan.fields[0]?.label ?? "Antes"}
                    value={actionBefore.state}
                    onChange={actionBefore.setState}
                    placeholder={exodo16Lesson.actionPlan.fields[0]?.placeholder ?? ""}
                    rows={6}
                    tone="spiritual"
                  />
                  <InputArea
                    label={exodo16Lesson.actionPlan.fields[1]?.label ?? "Durante"}
                    value={actionDuring.state}
                    onChange={actionDuring.setState}
                    placeholder={exodo16Lesson.actionPlan.fields[1]?.placeholder ?? ""}
                    rows={6}
                    tone="spiritual"
                  />
                  <InputArea
                    label={exodo16Lesson.actionPlan.fields[2]?.label ?? "Depois"}
                    value={actionAfter.state}
                    onChange={actionAfter.setState}
                    placeholder={exodo16Lesson.actionPlan.fields[2]?.placeholder ?? ""}
                    rows={6}
                    tone="spiritual"
                  />
                </div>
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-sm text-slate-600">
                          {exodo16Lesson.referenceMaterial.title}
                        </div>
                      </div>
                  ...
                  <div className="mt-6 grid gap-5 lg:grid-cols-2">
                    <div className="rounded-3xl border border-slate-200 bg-sud-gray p-5 sm:p-6">
                      ...
                      <div className="mt-4 space-y-4">
                        {exodo16Lesson.referenceMaterial.sections.map((section) => (
                    ...
                    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                      ...
                      {exodo16Lesson.referenceMaterial.scriptureStudy ? (
                        <div className="mt-4">
                          <div className="text-sm font-bold text-slate-900">
                            {exodo16Lesson.referenceMaterial.scriptureStudy.title}
                          </div>
                          <div className="mt-2 flex flex-wrap gap-2">
                            {exodo16Lesson.referenceMaterial.scriptureStudy.passages.map(
                              (p) => (
                                <span
                                  key={p}
                          ...
                          <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-slate-700">
                            {exodo16Lesson.referenceMaterial.scriptureStudy.prompts.map(
                              (q) => (
                                <li key={q}>{q}</li>
                              ),
                            )}
                      ...
                      <div className="mt-6">
                        <a
                          href={exodo16Lesson.referenceMaterial.sourceUrl}
                          target="_blank"
                          rel="noreferrer"
